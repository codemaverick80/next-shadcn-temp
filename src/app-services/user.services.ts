...
import {createGoogleUser, getUserByEmail} from "@/app-db-access/user.data-access";
...
export async function createGoogleUserService(googleUser: GoogleUser) {
    ...
    await createProfile(existingUser.id, googleUser.name, googleUser.picture, googleUser.given_name, googleUser.family_name);
